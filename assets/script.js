...
const programContents = [
  { image: 'assets/img/program-exhibition.webp', category: '전시', title: '기억의 기록전', description: '사진과 문서, 영상과 목소리를 통해 기억이 세대를 넘어 전해지는 과정을 살펴보는 전시입니다.' },
  { image: 'assets/img/program-letter.webp', category: '참여', title: '내일에게 보내는 편지', description: '미래의 누군가에게 전하고 싶은 평화의 메시지를 작성해 아카이브에 남기는 참여 프로그램입니다.' },
  { image: 'assets/img/program-passport.webp', category: '미션', title: '피스 패스포트', description: '행사장 네 개 공간의 프로그램에 참여하고 스탬프를 모으면 기념 평화 엽서를 받을 수 있습니다.' },
  { image: 'assets/img/program-badge.webp', category: '창작', title: '평화 배지 랩', description: '평화를 의미하는 여러 언어와 그래픽 요소를 조합해 나만의 배지를 만드는 프로그램입니다.' },
  { image: 'assets/img/program-mural.webp', category: '창작', title: '평화 그림 함께 그리기', description: '점과 선, 교차점을 활용해 시민이 함께 하나의 대형 평화 그림을 완성합니다.' },
  { image: 'assets/img/program-culture.webp', category: '체험', title: '글로벌 문화체험', description: '세계 여러 지역의 인사말과 생활문화를 체험하며 서로의 차이를 이해하는 참여 공간입니다.' },
  { image: 'assets/img/program-photo.webp', category: '포토', title: '피스 포토존', description: '축제의 열린 프레임과 연결된 선을 활용해 구성한 공식 촬영 구간입니다.' }
];
...
const programFeatureCategory = programFeature?.querySelector('h3');
const programFeatureTitle = programFeature?.querySelector('h2');
...
const activateProgram = async (item, index) => {
  ...
  const isAccordion = window.matchMedia('(max-width: 1100px)').matches;
  if (isAccordion) {
    const currentActive = programItems.find(programItem => programItem.classList.contains('is-active'));
    currentActive?.classList.remove('is-detail-open');
    window.clearTimeout(programAccordionTimer);
    programFeature.classList.add('is-accordion-collapsed');
    const collapseDuration = window.matchMedia('(prefers-reduced-motion: reduce)').matches ? 0 : 460;
    await new Promise(resolve => {
      programAccordionTimer = window.setTimeout(resolve, collapseDuration);
    });
  ...
  if (isAccordion) item.classList.add('is-detail-open');
  ...
  window.requestAnimationFrame(() => {
    programFeature.classList.remove('is-loading');
    if (isAccordion) programFeature.classList.remove('is-accordion-collapsed');
  });
};
